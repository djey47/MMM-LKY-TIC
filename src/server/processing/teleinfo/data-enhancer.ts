...
function computeSuppliedPowers(data: TeleInfo): { currentDay?: number[]; total?: number[]; } | undefined {
  ...
  const initialDayIndexes = storeInstance.get(currentDayIndexesISKey) as number[];
  ...
  if (currentDay) {
    const currentDaySuppliedISKey = generateCurrentDayISKey(PER_DAY_SUPPLIED_IS_KEY_PREFIX);
...
function computeSuppliedPower(initialIndexes?: (number|undefined)[], currentIndexes?: (number|undefined)[]) {
