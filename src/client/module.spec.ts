...
describe('MM2 Module client', () => {
  ...
  describe('getStyles overriden function', () => {
    ...
    it('should return correct styles when file utility not available', () => {
      // given
      const { implementation } = checkAndExtractRegistration(
        mockModuleRegister.mock.lastCall
      );
  ...
  describe('start overriden function', () => {
    ...
    it('should set global state and perform initializations', () => {
      ...
      expect(mockCatcherGetInstance).toHaveBeenCalledWith({
        isDebugMode: false,
      });
    });
  ...
  describe('getDom overriden function', () => {
    ...
    it('should return wrapper div when react render engine is not started', () => {
      ...
      expect(mockRenderWrapper).toHaveBeenCalledWith('mmm-lky-tic-wrapper');
    });
  });
...
function checkAndExtractRegistration(call?: unknown) {
  ...
  const implementation = mockModuleRegister.mock
    .lastCall[1] as MM2ModuleProperties;
  ...
  // Make use of this
  enhancedImplementation.getStyles = enhancedImplementation.getStyles.bind(
    enhancedImplementation
  );
  enhancedImplementation.start = enhancedImplementation.start.bind(
    enhancedImplementation
  );
